...
MenuState.prototype.init = function () {
    ...
    this.title.components[FM.ComponentTypes.RENDERER].setFormat('#f00', '60px sans-serif bold', 'middle');
    //Add the title to the MenuState to have the engine display and update it
    this.add(this.title);
    //Add a spatial component to the title to specify a position
    this.instructions.addComponent(new FM.SpatialComponent(300, 350, this.instructions));
    //Add a text renderer component to the title to display a text
    this.instructions.addComponent(new FM.TextRendererComponent("Press SPACE or click to start", this.instructions));
    this.instructions.components[FM.ComponentTypes.RENDERER].setFormat('#000', '30px sans-serif', 'middle');
    //Add the title to the MenuState to have the engine display and update it
    this.add(this.instructions);
    this.tuto.addComponent(new FM.SpatialComponent(200, 550, this.tuto));
    this.tuto.addComponent(new FM.TextRendererComponent("| WQSD to move |", this.tuto));
    this.tuto.components[FM.ComponentTypes.RENDERER].setFormat('#000', '30px sans-serif', 'middle');
    this.add(this.tuto);
    this.tuto = new FM.GameObject(10);
    this.tuto.addComponent(new FM.SpatialComponent(200, 600, this.tuto));
    this.tuto.addComponent(new FM.TextRendererComponent("| Short click to shoot a slow bullet |", this.tuto));
    this.tuto.components[FM.ComponentTypes.RENDERER].setFormat('#000', '30px sans-serif', 'middle');
    this.add(this.tuto);
    this.tuto = new FM.GameObject(10);
    this.tuto.addComponent(new FM.SpatialComponent(200, 650, this.tuto));
    this.tuto.addComponent(new FM.TextRendererComponent("| Long click to shoot a fast bullet |", this.tuto));
    this.tuto.components[FM.ComponentTypes.RENDERER].setFormat('#000', '30px sans-serif', 'middle');
    this.add(this.tuto);
};
/**
...
MenuState.prototype.update = function (dt) {
    ...
    if (FM.Game.isKeyPressed(FM.Keyboard.SPACE) || FM.Game.isMouseClicked()) {
